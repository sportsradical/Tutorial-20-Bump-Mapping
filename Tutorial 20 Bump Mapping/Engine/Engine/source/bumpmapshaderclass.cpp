...
bool BumpMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/source/bumpmap.vs", L"../Engine/source/bumpmap.ps");
	if(!result)
	{
		return false;
	}
								...
								XMFLOAT4 diffuseColor)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, difuseMap, bumpMap, lightDirection,
								 diffuseColor);
	if(!result)
	{
		return false;
											 ...
											 XMFLOAT4 diffuseColor)
{
	...
	deviceContext->PSSetShaderResources(0, 1, difuseMap);
